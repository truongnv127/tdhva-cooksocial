...
/**
 * Whitelist of allowed modules to prevent code injection
 */
const ALLOWED_MODULES = ['custom'];

/**
 * The names of modules to load are stored as a comma-delimited string in the
 * `MODULES` env var.
 */
const moduleNames = (process.env.MODULES || '').split(',').filter(name => {
  const trimmedName = name.trim();
  return trimmedName && ALLOWED_MODULES.includes(trimmedName);
});
